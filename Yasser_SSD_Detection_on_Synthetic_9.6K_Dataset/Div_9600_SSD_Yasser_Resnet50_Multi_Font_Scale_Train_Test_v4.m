...
gpuDevice(1)
% return;
Yassers_Model='Yasser_9600_v1_Urdu_SSD_Trained_On_';
%%
% Load training data.

data = load('Training_Data_Regression_CLE_Multi_Font_v1_Ysr_Rotation_CLE_DataSet_v1_Multi_Font_Ysr_NoOf_Images_9600_2_3_4_4_100_.mat');
...
trainingData3=trainingData2;
imageRange=size(trainingData3,1);
% % % % disp('Estimating Anchorboxes values from data..... (wait a little)');
...
blds = boxLabelDatastore(trainingData3(startI:imageRange,2));
...
%             'CheckpointPath', tempdir);
        ...
        options = trainingOptions('sgdm',...
          ...
          'Shuffle','every-epoch',...
          'VerboseFrequency',25,...
          'CheckpointPath',[pwd '\ssd_9.6K_Training']);
...
% % % % % %                                 SampleWritePath='Auto_Testing_OutPut_Few_Images';
% % % % % %                                 a = getframe;
% % % % % %                                 pause(0.05);
% % % % % %                                 ImgWritingPath=[fullfile(pwd,SampleWritePath) '\' name ext];
% % % % % %                                 pause(0.1);
% % % % % %                                 cData_beebo=a.cdata;
