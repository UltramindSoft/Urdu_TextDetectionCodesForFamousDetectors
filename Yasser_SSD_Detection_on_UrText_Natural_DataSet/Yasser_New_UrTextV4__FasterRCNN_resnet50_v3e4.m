...
New_Images_Path='E:\PhD2_Yasser_v2\Detection_Results_For_Papers\Yasser_SSD_Detection_Natural_DataSet\New_Volume_2b_images__930X1240_UrTextV4';
...
%                 data=load('faster_rcnn_stage_4_checkpoint__17820__2020_09_13__22_05_35.mat');
                data=load('faster_rcnn_stage_4_checkpoint__5346__2020_09_14__04_11_59.mat');
                tic
        ...
        % Ysr_Model_Epochs=Ysr_Model_Epochs+10;                          
...
% load('Yasser_Natural_UrTextV4_930X1240_Urdu_FasterRCNN_FaisalComputer_Trained_On_1863_Tested_On_207-images_n_Model-Name_resnet18_Ep15_Tr_ap-0.5547_Ts_ap-0.43522_5283.9263_.mat', 'detector')
...
%                 for i = 1:numImages/100
                for i = 1:numImages/1
                               spath=[fullfile(pwd,'FasterRCNN_Test_Detection_Results') '\FasterRCNN_Original_images__930X1240_UrTextV4' num2str(i)];
                               I =imread(testData.Original_Outodoor_Image_Path{i});
                                imshow(I);
                                drawnow
                                 f = gcf;
                                % Requires R2020a or later
                                exportgraphics(f,[spath '.png'],'Resolution',300);
                                pause(0.1);
                                [bboxes,scores] = detect(detector,I,'ExecutionEnvironment','gpu','MiniBatchSize',1,'NumStrongestRegions',30);
                                detectedImg = insertShape(I, 'Rectangle', bboxes,'Color','red','LineWidth',4);
                                imshow(detectedImg,'InitialMagnification','fit');
                                drawnow 
                                pause(0.1);
                                results(i).Boxes = bboxes;
                                results(i).Scores = scores;
                                ...
                                spath=[fullfile(pwd,'FasterRCNN_Test_Detection_Results') '\FasterRCNN_Detections_images__930X1240_UrTextV4' num2str(i)];
